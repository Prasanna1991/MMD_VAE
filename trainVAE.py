...
for it in range(10):
    ...
    for i in range(0, sampleEachEpoch, mb_size):
        ...
        #Forward data
        px1, px2, zm1, zm2, zlogvar1, zlogvar2 = model(X1, X2)

        #Calculate loss
        reconLoss, KLLoss = loss_function(px1, px2, X1, X2, zm1, zm2, zlogvar1, zlogvar2)

        #Backward
        loss = (reconLoss + KLLoss) / 2
        loss.backward()
