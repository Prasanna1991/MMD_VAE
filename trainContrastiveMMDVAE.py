...
def loss_function(recon_x1, recon_x2, x1, x2, mu1, mu2, logvar1, logvar2, VTlabel, PAlabel, dist, z1, z2):
    x1 = x1.float()
    x2 = x2.float()
    ...
    errHingeVT = hinge_loss(dist, PAlabel)
    return reconLoss, KLLoss, mmdLoss, errHingeVT
...
for it in range(10):
    ...
    for i in range(0, sampleEachEpoch, mb_size):
        ...
        #     PAAlabel[k] = qrsPairs[shuffle[j]][3]
        #     k = k + 1
        #
        ...
        # PAlabel = Variable(torch.from_numpy(PAAlabel.astype('float32')))
        ...
        PAlabel = Variable(torch.ones(mb_size)) #Similarly PAlabel in our real dataset is in the form of 1 and -1.
        if cuda:
            X1 = X1.cuda()
            X2 = X2.cuda()
            VTlabel = VTlabel.cuda()
            PAlabel = PAlabel.cuda()

        #zero grad
        solver.zero_grad()

        # Forward data
        _, z1, zm1, zlogvar1, px1, _, z2, zm2, zlogvar2, px2, dist = model(X1, X2)

        # Calculate loss
        reconLoss, KLLoss, mmdLoss, hingeLoss = loss_function(px1, px2, X1, X2, zm1, zm2, zlogvar1, zlogvar2,
                                                              VTlabel, PAlabel, dist, z1, z2)

        # Backward
        loss = (reconLoss + 1 * KLLoss) / 2 +  mmdLoss  + 1 * hingeLoss
        loss.backward()
    ...
    if it % 5 == 0:
        ...
        print('Iter-{}; hingeLoss: {:.4}'.format(it, hingeLoss.data[0]))

    if it % 50 == 0:
        torch.save(model.state_dict(), 'model_contrastiveMMDVAE.pt')
        totalLoss.append(loss.data[0])
        np.save('contrastiveMMDVAELoss.npy', totalLoss)
